...
export default function SignInContent() {
  ...
  useEffect(() => {
    const insertGoogleButton = () => {
      ...
      if (identifierFieldRow && googleButton) {
        // 이미 삽입되어 있는지 확인
        const parent = googleButton.parentElement;
        if (
          parent &&
          parent.contains(identifierFieldRow) &&
          identifierFieldRow.previousSibling === googleButton
        ) {
          // 이미 올바른 위치에 있으면 리턴
          return;
        }
        ...
        console.log("[SignInContent] 구글 버튼을 맨 위에 삽입");
        ...
        if (formContainer) {
          formContainer.insertBefore(googleButton, identifierFieldRow);
        } else {
    ...
    // 초기 실행 및 주기적 확인
    const initialTimeout = setTimeout(insertGoogleButton, 400);
  ...
  useEffect(() => {
    const insertKakaoButton = () => {
      ...
      if (identifierFieldRow && googleButton && kakaoButton) {
        // 이미 삽입되어 있는지 확인
        const parent = kakaoButton.parentElement;
        if (
          parent &&
          parent === googleButton.parentElement &&
          googleButton.nextSibling === kakaoButton
        ) {
          // 이미 올바른 위치에 있으면 리턴
          return;
        }
        ...
        console.log("[SignInContent] 카카오 버튼을 구글 버튼 아래에 삽입");
        ...
        // 구글 버튼 다음에 삽입
        if (googleButton.parentElement) {
          googleButton.parentElement.insertBefore(
            kakaoButton,
            googleButton.nextSibling,
          );
        } else {
          // 구글 버튼이 없으면 이메일 필드 위에 삽입
          const formContainer = identifierFieldRow.parentElement;
          if (formContainer) {
