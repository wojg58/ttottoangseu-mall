...
### "GW.IP_NOT_ALLOWED" 오류

이 오류는 **서명/키 문제가 아니라 IP 허용(Whitelist) 미등록**일 때 발생합니다.

#### 로컬에서 공인 IP 확인

아래 중 하나로 공인 IP를 확인하세요:

- `https://api.ipify.org` 접속 (화면에 숫자만 표시)
- `https://ifconfig.me` 접속

#### 허용 IP 등록 위치

1. **커머스API센터 > 내 스토어 애플리케이션** 접속
2. 해당 앱 선택
3. **허용 IP(Whitelist)** 항목에 공인 IP 추가
4. 저장

#### 주의사항

- 공인 IP는 **네트워크가 바뀌면 변경**될 수 있습니다.
- IP가 바뀌면 **다시 GW.IP_NOT_ALLOWED가 발생**할 수 있습니다.
